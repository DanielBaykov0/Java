...
public class Main {
    public static void main(String[] args) {
        ...
        IntFunction<String> consoleReader = ignored -> scanner.nextLine();

        List<Person> people = IntStream
                .range(0, Integer.parseInt(scanner.nextLine()))
                .mapToObj(consoleReader)
                .map(Person.STRING_PERSON_FUNCTION)
    ...
    private static final class Person {
        public static final Function<String, Person> STRING_PERSON_FUNCTION = input -> {
            String[] tokens = Objects.requireNonNull(input, "Null person input").split(", ");
